...
class Config implements \ArrayAccess
{
     ...
     * @return mixed
     */
    public function parse($config, $type = '', $name = '') //config/app.php ,php,app
    {
        if (empty($type)) {
            $type = pathinfo($config, PATHINFO_EXTENSION);
        }

        $object = Loader::factory($type, '\\think\\config\\driver\\', $config); //php ， ，config/app.php
     ...
     * @return mixed
     */
    public function load($file, $name = '')  //两个参数config/app.php , app
    {
        if (is_file($file)) {
            $filename = $file;
    ...
    protected function loadFile($file, $name)//两个参数config/app.php , app
    {
     ...
     /*   if ('php' == $type) {
        ...
        return $this->parse($file, $type, $name);  //config/app.php ,php,app
    }
