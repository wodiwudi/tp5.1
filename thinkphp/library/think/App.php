...
class App extends Container
{
    ...
    public function init($module = '')
    {
        ...
        if (is_file($path . 'init.php')) {
            ...
            // 自动读取配置文件
            if (is_dir($path . 'config')) {     //源文件目录下的config
                $dir = $path . 'config' . DIRECTORY_SEPARATOR;
            } elseif (is_dir($this->configPath . $module)) { //对应的模块配置的位置
                $dir = $this->configPath . $module;
            }

            $files = isset($dir) ? scandir($dir) : [];   //$files = config目录所有.php配置文件

            foreach ($files as $file) {
                if ('.' . pathinfo($file, PATHINFO_EXTENSION) === $this->configExt) {
                    $this->config->load($dir . $file, pathinfo($file, PATHINFO_FILENAME));  //两个参数config/app.php , app
                }
            }
        }
